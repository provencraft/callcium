...
| EQ_CTX | 32 | `[contextPropertyId:32]` | Context property whose value is the comparison operand (Section 5.4) |
...
5. Evaluate groups in stored order; the first passing group satisfies the policy (OR semantics).
6. Within each group, all rules must pass (AND semantics). Rules evaluate in stored order; the first rule that produces a violation ends the group's evaluation, and only that violation's effect (Section 9.3) applies.
...
Only violations reached by the evaluation order of Section 7.1 have an effect: a group's first violation fixes that group's outcome, and whether a later rule of the same group would have produced a group-local or an abort violation never influences the verdict.

Enforcers MUST produce the same accept/reject verdict for identical policy, calldata, and context. Reporting MAY differ: an enforcer MAY evaluate additional rules or groups to collect diagnostics, but such evaluation has no verdict effect.
