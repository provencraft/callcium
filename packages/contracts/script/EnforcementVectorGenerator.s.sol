...
contract EnforcementVectorGenerator is Script {
    ...
    function run() external {
        ...
        // Value canonicalization: a word outside its declared encoding is rejected, with a canonical control.
        _vectorCanonUintDirtyReject();
        _vectorCanonUintCanonicalPass();
        _vectorCanonSignedNoExtReject();
        _vectorCanonSignedCanonicalPass();
        _vectorCanonBytesNDirtyReject();
        _vectorCanonBytesNCanonicalPass();
        _vectorCanonBoolTwoReject();
        _vectorCanonBoolCanonicalPass();
        _vectorCanonAddressDirtyReject();
        _vectorCanonAddressCanonicalPass();
        _vectorCanonFunctionDirtyReject();
        _vectorCanonFunctionCanonicalPass();
    ...
    /// @dev uint64 `>= 1000`: low 64 bits = 2000, bit set above the width.
    function _vectorCanonUintDirtyReject() private {
        ...
        _addErrorVector(
            "canon-uint-dirty-reject",
            "uint64 word with bits above the width is rejected",
            policy,
            callData,
            PolicyEnforcer.NonCanonicalValue.selector,
            "NON_CANONICAL_VALUE"
        );
    }

    /// @dev Control for the above: the same value with a clean high half satisfies the bound.
    function _vectorCanonUintCanonicalPass() private {
        bytes memory policy = PolicyBuilder.create("foo(uint64)").add(arg(0).gte(uint256(1000))).build();
        bytes memory callData = abi.encodePacked(bytes4(keccak256("foo(uint64)")), bytes32(uint256(2000)));
        _addVector("canon-uint-canonical-pass", "Canonical uint64 2000 satisfies >= 1000", policy, callData, true);
    }

    /// @dev int64 `>= 0`: low 64 bits = 5, bits set above the width instead of sign extension.
    function _vectorCanonSignedNoExtReject() private {
        ...
        _addErrorVector(
            "canon-signed-noext-reject",
            "int64 word that is not sign-extended is rejected",
            policy,
            callData,
            PolicyEnforcer.NonCanonicalValue.selector,
            "NON_CANONICAL_VALUE"
    ...
    /// @dev Control for the above: a sign-extended negative word evaluates against the bound.
    function _vectorCanonSignedCanonicalPass() private {
        bytes memory policy = PolicyBuilder.create("foo(int64)").add(arg(0).gte(int256(0))).build();
        bytes memory callData = abi.encodePacked(bytes4(keccak256("foo(int64)")), bytes32(uint256(5)));
        _addVector("canon-signed-canonical-pass", "Canonical int64 5 satisfies >= 0", policy, callData, true);
    }

    /// @dev bytes4 `== 0x11223344`: value in the high 4 bytes, non-zero padding bytes below it.
    function _vectorCanonBytesNDirtyReject() private {
        ...
        _addErrorVector(
            "canon-bytesn-dirty-reject",
            "bytes4 word with non-zero padding is rejected",
            policy,
            callData,
            PolicyEnforcer.NonCanonicalValue.selector,
            "NON_CANONICAL_VALUE"
    ...
    /// @dev Control for the above: zero padding leaves the equality intact.
    function _vectorCanonBytesNCanonicalPass() private {
        bytes32 operand = bytes32(uint256(0x11223344) << 224);
        bytes memory policy = PolicyBuilder.create("foo(bytes4)").add(arg(0).eq(operand)).build();
        bytes memory callData = abi.encodePacked(bytes4(keccak256("foo(bytes4)")), operand);
        _addVector("canon-bytesn-canonical-pass", "Canonical bytes4 matches the operand", policy, callData, true);
    }

    /// @dev bool `== true`: word equal to 2, outside the single-bit value width.
    function _vectorCanonBoolTwoReject() private {
        bytes memory policy = PolicyBuilder.create("foo(bool)").add(arg(0).eq(true)).build();
        bytes memory callData = abi.encodePacked(bytes4(keccak256("foo(bool)")), bytes32(uint256(2)));
        _addErrorVector(
            "canon-bool-two-reject",
            "bool word outside 0 and 1 is rejected",
            policy,
            callData,
            PolicyEnforcer.NonCanonicalValue.selector,
            "NON_CANONICAL_VALUE"
    ...
    /// @dev Control for the above: the canonical encoding of true satisfies the equality.
    function _vectorCanonBoolCanonicalPass() private {
        bytes memory policy = PolicyBuilder.create("foo(bool)").add(arg(0).eq(true)).build();
        bytes memory callData = abi.encodePacked(bytes4(keccak256("foo(bool)")), bytes32(uint256(1)));
        _addVector("canon-bool-canonical-pass", "Canonical bool 1 satisfies == true", policy, callData, true);
    }

    /// @dev address `== 0x4D2`: value in the low 160 bits, bits set above them.
    function _vectorCanonAddressDirtyReject() private {
        ...
        _addErrorVector(
            "canon-address-dirty-reject",
            "address word with bits above 160 is rejected",
            policy,
            callData,
            PolicyEnforcer.NonCanonicalValue.selector,
            "NON_CANONICAL_VALUE"
    ...
    /// @dev Control for the above: the same address with a zero high half matches.
    function _vectorCanonAddressCanonicalPass() private {
        address target = address(uint160(0x4d2));
        bytes memory policy = PolicyBuilder.create("foo(address)").add(arg(0).eq(target)).build();
        bytes memory callData = abi.encodePacked(bytes4(keccak256("foo(address)")), bytes32(uint256(uint160(target))));
        _addVector("canon-address-canonical-pass", "Canonical address matches the operand", policy, callData, true);
    }

    /// @dev function `==` (bytes24-aligned): value in the high 24 bytes, non-zero low padding.
    function _vectorCanonFunctionDirtyReject() private {
        ...
        _addErrorVector(
            "canon-function-dirty-reject",
            "function word with non-zero padding is rejected",
            policy,
            callData,
            PolicyEnforcer.NonCanonicalValue.selector,
            "NON_CANONICAL_VALUE"
    ...
    /// @dev Control for the above: zero low padding leaves the equality intact.
    function _vectorCanonFunctionCanonicalPass() private {
        uint256 fnValue = uint256(0x0102030405060708090a0b0c0d0e0f101112131415161718) << 64;
        bytes memory policy = PolicyBuilder.create("foo(function)").add(arg(0).eq(bytes32(fnValue))).build();
        bytes memory callData = abi.encodePacked(bytes4(keccak256("foo(function)")), bytes32(fnValue));
        _addVector("canon-function-canonical-pass", "Canonical function value matches the operand", policy, callData, true);
    }
