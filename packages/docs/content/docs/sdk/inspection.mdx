...
`groups` is in the policy's canonical order — `build()` sorts groups by hash — so array indices do not correspond to `.or()` authoring order.
...
- `DecodedRule`: one constraint (scope, path, compiled hint, opCode, operand data), each as a `Field<T>`.
...
parsePathSteps("0xfffe0001"); // [65534, 1] — Quantifier.ANY on arg path, next step 1
```
