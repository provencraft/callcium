...
`Quantifier.ALL` and `Quantifier.ANY` replace a concrete element index to apply a constraint across the entire array. Import them from `@callcium/sdk`:
...
| ---------------- | ------------------------------------------------ | ----------- |
| `Quantifier.ALL` | All elements must satisfy the constraint         | Passes      |
| `Quantifier.ANY` | At least one element must satisfy the constraint | Fails       |

`Quantifier.ALL` is vacuously true on an empty array. When an empty array must be rejected, add a length constraint to the same group: `lengthGt(0n)` alongside the `ALL` constraint.
...
const policy = PolicyBuilder
  ...
  .add(arg(0).lengthBetween(1n, 32n))          // bounded batch size
