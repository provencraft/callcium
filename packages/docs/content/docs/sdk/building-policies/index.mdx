...
Address values are passed as `0x`-prefixed strings; numeric and `bytesN` values as `bigint`.
...
**Type restriction:** accepts arrays of 32-byte static values: addresses (`0x`-prefixed strings), and `uint*`/`int*`/`bytes*` (as `bigint`). The builder sorts and deduplicates the set automatically. The set must have at least one element and no more than 2,047.
...
Narrow numeric types (`uint16`, `int32`, and similar) are passed as `bigint` directly. A `bytesN` value occupies the leading bytes of its 32-byte word, so shift it into that position — an operand that is not the canonical encoding for the declared type is rejected.

```ts
// bytes4 occupies the leading 4 bytes of the word.
const allowedSelectors = [
  BigInt("0xa9059cbb") << 224n, // transfer
  BigInt("0x095ea7b3") << 224n, // approve
];
...
const policy = PolicyBuilder
  ...
  .add(arg(1).lengthBetween(4n, 256n)) // data
  .build();
```

`lengthBetween(4n, 256n)` ensures the payload is at least 4 bytes and imposes a hard upper bound on untrusted input size.

## Transaction context

The following context properties are available as constraint targets:
...
| `baseFee()`        | Base fee of the block.                   | `uint256` |
| `gasPrice()`       | Effective gas price of the transaction.  | `uint256` |
...
Onchain, `msgSender` resolves in the frame that runs enforcement: a router or aggregator that reaches the enforcing contract with `call` is the sender, while `delegatecall` preserves the original caller. Simulate flows that pass through an intermediate contract with `msgSender` set to that contract's address.

`baseFee` and `gasPrice` are constraints on network state: the same data is valid or invalid against the policy as gas prices move. The gas price is additionally chosen by the transaction sender, bounded below by the base fee.
...
`eqCtx` and `neqCtx` compare a value against a context property instead of a literal. The operand names the property by ID; the property's value resolves at enforcement time.

```ts
import { PolicyBuilder, arg, ContextProperty } from "@callcium/sdk";

// function transfer(address recipient, uint256 amount)
const policy = PolicyBuilder
  .create("transfer(address,uint256)")
  .add(arg(0).eqCtx(ContextProperty.MSG_SENDER)) // recipient == caller
  .build();
```

Address properties (`MSG_SENDER`, `TX_ORIGIN`) pair with `address` arguments; uint256 properties pair with unsigned integer arguments. Context targets accept the operator too: `msgSender().eqCtx(ContextProperty.TX_ORIGIN)` constrains two properties against each other.

## Array quantifiers

`Quantifier.ALL` and `Quantifier.ANY` apply a constraint across all elements of an array parameter:
...
| Context reference | `eqCtx`, `neqCtx`                                                           | `address` for address properties; `uint*` for uint256 properties         |
