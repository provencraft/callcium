...
[Git Source](https://github.com/provencraft/callcium/blob/main/packages/sdk/src/policy-enforcer.ts#L637)
...
```ts
function check(policy: `0x${string}`, callData: `0x${string}`, context?: Context): EnforceResult;
```
...
```ts
function enforce(policy: `0x${string}`, callData: `0x${string}`, context?: Context): void;
```
