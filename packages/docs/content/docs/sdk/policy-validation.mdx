...
To encode a policy despite reported issues, use `buildUnsafe()`. It skips validation entirely — the deliberate escape hatch for policies that target an enforcer with different operational limits than the reference Solidity enforcer:
...
| `compatibility` | A construct exceeds the reference enforcer's operational limits, or is a known semantic footgun (e.g. `neq` under `any()`). |
