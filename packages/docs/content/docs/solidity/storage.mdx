...
`PolicyManager` emits an event for every storage mutation: `PolicyStored(policyHash, pointer)`, `PolicyBound(target, selector, policyHash)`, and `PolicyUnbound(target, selector)`. Binding and unbinding change what the enforcer accepts — the events make that configuration indexable and monitorable offchain.

`PolicyStored` fires on every store call, including deduplicated stores of an already-known blob. `PolicyBound` carries the selector derived from the policy. Custom managers built directly on `PolicyRegistry` define their own events; the library emits nothing.
