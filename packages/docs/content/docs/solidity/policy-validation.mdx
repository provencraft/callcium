...
Callcium validates policies at build time through semantic analysis. When you call `PolicyBuilder.build()`, the [`PolicyValidator`](/docs/solidity/reference/policy-validator) inspects every constraint for type errors, logical contradictions, redundancies, and vacuous conditions — then reverts if any issue is found.
...
| `value1`          | `bytes32`       | First value involved in the issue.                           |
| `value2`          | `bytes32`       | Second value involved in the issue.                          |
...
Operators, operands, and paths that do not fit the descriptor's declared types. All are **Error** severity.

| Code                        | Description                                                          |
| --------------------------- | -------------------------------------------------------------------- |
| `VALUE_OP_ON_DYNAMIC`       | Value operator used on a dynamic type.                               |
| `VALUE_OP_ON_COMPOSITE`     | Value operator used on a composite type.                             |
...
| `IN_ON_BOOL`                | `isIn`/`notIn` used on a `bool`.                                     |
| `LENGTH_ON_STATIC`          | Length operator used on a non-dynamic type.                          |
| `CONTEXT_TYPE_MISMATCH`     | Context operand paired with a target type its property cannot match. |
| `UNKNOWN_OPERATOR`          | Unrecognized operator code.                                          |
| `NON_CANONICAL_OPERAND`     | Operand word is not the canonical encoding for the declared type.    |
| `UNNAVIGABLE_PATH`          | Path does not navigate the descriptor.                               |
| `NESTED_QUANTIFIER`         | Path applies more than one quantifier.                               |
| `HINT_MISMATCH`             | Stored hint differs from the path compiled against the descriptor.   |
...
| `FUSIBLE_RANGE`          | `gte()` and `lte()` fuse into a single `between()`.                   |
| `FUSIBLE_LENGTH_RANGE`   | `lengthGte()` and `lengthLte()` fuse into a single `lengthBetween()`. |
...
| `VACUOUS_NEGATED_RANGE`        | Negated `between()` with low > high excludes nothing.       |
| `VACUOUS_NEGATED_LENGTH_RANGE` | Negated `lengthBetween()` with low > high excludes nothing. |

### Compatibility

Constructs that cross a normative limit or that behave differently than most authors intend. `QUANTIFIER_OVER_STATIC_LIMIT` is **Error** severity; the rest are **Warning**. Normative limits are fixed for this format version and every conformant enforcer applies the same values, so `buildUnsafe()` is the only way to emit a policy that crosses one.

| Code                           | Description                                                                     |
| ------------------------------ | ------------------------------------------------------------------------------- |
| `PATH_DEPTH_EXCEEDED`          | Path deeper than `MAX_PATH_DEPTH`; a policy declaring it is not well-formed (PWF-17).  |
| `QUANTIFIER_OVER_STATIC_LIMIT` | Quantifier over a static array longer than `MAX_QUANTIFIED_ARRAY_LENGTH` (PV-7).       |
| `UNKNOWN_CONTEXT_PROPERTY`     | Context property ID outside the assigned set (PWF-25).                                 |
| `NEGATION_UNDER_ANY`           | Negated operator under `any()`; a single decoy element satisfies the rule.             |
