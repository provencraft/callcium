...
Both revert on a malformed policy, and both revert on a violation that aborts evaluation. Only `enforce` reverts on a violation that merely fails its group.
...
The match covers the selector alone. The embedded descriptor carries argument types but no function name, so a policy's selector and its descriptor are related only by the signature string given to `create()` — the enforcer cannot detect a descriptor authored for a different function with the same argument types.
...
**Revert when unbound.** If `_resolvePolicy` returns empty bytes, the call is blocked. A missing, unbound, or mis-keyed policy surfaces as a revert instead of running the function unguarded.
...
**Skip when unbound.** If `_resolvePolicy` returns empty bytes, enforcement is skipped and the function runs unguarded. An absent guard produces no signal; use this only where unguarded execution is the intended default.
...
**Groups use OR semantics.** The enforcer evaluates groups in canonical order — `build()` sorts groups by group hash, so the evaluation order is deterministic but independent of authoring order. The first group that passes ends evaluation — the data is valid against the policy. If every group fails, `PolicyViolation` is thrown.
...
Each call to `.or()` in the builder starts a new group, but group order on the wire is canonical, not authored: a group written first can evaluate last. Ordering does not affect the verdict — it determines which group an error reports, and where evaluation stops when a group aborts on malformed calldata.
...
The error reflects a single point of failure, not a complete trace. If a policy has multiple groups, the indices only reflect the failure in the final group; earlier group failures are discarded as the enforcer advances. Group indices follow the canonical group order, not `.or()` authoring order.
...
| `NonCanonicalValue`            | A resolved word is not the canonical encoding of its type. |
